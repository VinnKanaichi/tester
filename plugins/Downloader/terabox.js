...
const handler = async (m, { hanz, args }) => {
  if (!args[0]) return hanz.reply(m.chat, "URL tidak boleh kosong", m);
  const regex = /\/([A-Za-z0-9_-]+)$/;
  const match = args[0].match(regex);
  if (!match) return hanz.reply(m.chat, "Format URL tidak didukung", m);
  ...
  try {
    ...
    if (fileInfo.ok) {
      ...
      return hanz.reply(m.chat, "Terjadi kesalahan saat mengunduh file", m);
    }
  } catch (error) {
    return hanz.reply(m.chat, "Terjadi kesalahan", m);
  }
};
