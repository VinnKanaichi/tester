let handler = async (m, { hanz,setReply, args, command }) => {
  try {
    ...
    if (!args[0] || isNaN(args[0])) {
      return conn.reply(m.chat, `Masukkan jumlah saldo yang ingin ditransfer.\nContoh: .${command} 100`, m);
    }
    ...
    return hanz.reply(
      m.chat,
      `✅ Berhasil mentransfer saldo sebesar Rp.${amount.toLocaleString()} ke nomor ${receiver.split("@")[0]}.`,
      m
    );
  } catch (err) {
    console.error(err);
