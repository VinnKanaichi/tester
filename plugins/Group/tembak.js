...
const handler = async (m, { hanz, usedPrefix, text, participants, groupMetadata }) => {
    if (!text) {
    ...
    try {
        ...
        if (!users) return hanz.reply(m.chat, `*_Doi Atau Nomor Tidak Ditemukan, Mungkin Sudah Keluar Atau Bukan Anggota Grup Ini Dan Tidak Ada Di Database Bot Silakan Daftar Terlebih Dahulu Ketik .daftar*_`, m);
        if (user === m.sender) return hanz.reply(m.chat, `_*Tidak Bisa Berpacaran Dengan Diri Sendiri.*_`, m);
        if (user === hanz.user.jid) return hanz.reply(m.chat, `_*Maaf Kak, Tapi Saya Sepenuhnya Milik Xyroine. :')*_`, m);
