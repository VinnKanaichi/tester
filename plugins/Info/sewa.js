...
// Function to generate greeting based on the time
let ucapanWaktu;
if (timeWib < "06:00:00") {
...
// Message template for 'sewa' command
const dada = (prefix, pushname, ucapanWaktu) => {
    return Ehztext(`${ucapanWaktu} 
...
const handler = async (m, { hanz, command, prefix, pushname,setReply}) => {
    if (command === 'sewa' || command === 'sewabot') {
        // Generate the message using the function
...
handler.tags = ['info'];  
handler.command = /^(sewa|sewabot)$/i;  // Match 'sewa' or 'sewabot' commands
