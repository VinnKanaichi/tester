...
let handler = async (m, { args,prefix,hanz }) => {
    ...
    // Check if a valid Telegram sticker link is provided
    if (args[0] && args[0].match(/(https:\/\/t.me\/addstickers\/)/gi)) {
        let xeonresources = await Telesticker(args[0]);
        await m.reply(`Waitt Mengirim ${xeonresources.length} stiker...`);

        // Send stickers based on the number and chat type
...
// Function to fetch stickers from a Telegram sticker pack
async function Telesticker(url) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            // Loop through stickers and get their file URLs
