...
async function chord(query) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      // Fetching the search results based on the query
      ...
      // Fetching the detailed post by id
      ...
      // Extracting additional details from the post content
      let title = data.posts[0].title_plain;
      let author = data.posts[0].author_name || "Unknown";
      let image = data.posts[0].thumbnail || "No image available"; // Default to "No image available" if no image exists
